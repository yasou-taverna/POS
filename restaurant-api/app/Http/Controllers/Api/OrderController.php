...
class OrderController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'items.*.recipe_id' => 'nullable|integer',
            'items.*.name' => 'nullable|string|max:255',
            'items.*.name_he' => 'nullable|string|max:255',
        ...
        try {
            ...
            if (!empty($validated['client_order_id'])) {
                $existingOrder = Order::where('client_order_id', $validated['client_order_id'])->first();

                if ($existingOrder) {
                    $existingOrder->load(['orderItems.recipe']);
                    DB::commit();
            ...
            foreach ($validated['items'] as $item) {
                $recipe = !empty($item['recipe_id']) ? Recipe::find($item['recipe_id']) : null;
                $itemName = $item['name_he'] ?? $item['name'] ?? $recipe?->name ?? 'Item';
                $itemPrice = isset($item['price']) ? (float) $item['price'] : (float) ($recipe?->price ?? 0);
            ...
            if (isset($validated['subtotal'], $validated['tax'], $validated['total'])) {
